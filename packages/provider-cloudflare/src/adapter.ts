...
interface RecordResponse { id: string; name: string; type: string; content: string; ttl: number; proxied?: boolean; }
...
export class CloudflareAdapter implements DnsProvider {
  ...
  constructor(options: { token?: string; baseUrl?: string; fetchImpl?: typeof fetch; resolveDns?: (hostname: string, type: string) => Promise<string[]> }) {
  ...
  async observeRecord(zoneId: string, hostname: string, ctx: ProviderContext): Promise<DnsRecordObservation | null> {
    ...
    return { provider: 'cloudflare', id: data.id, zoneId, name: data.name, type: data.type, content: data.content, ttl: data.ttl, proxied: data.proxied === true, ownershipFingerprint: null };
  }
  ...
  async ensureRecord(zoneId: string, required: RequiredDnsRecord, ownershipFingerprint: string, ctx: ProviderContext): Promise<MutationResult<DnsRecordObservation>> {
    ...
    if (current && current.ownershipFingerprint !== null && current.ownershipFingerprint !== ownershipFingerprint) throw new ProviderRequestError({ code: 'LP-DNS-CONFLICT-UNOWNED', class: 'CONFLICT', provider: 'cloudflare', message: `DNS record '${required.hostname}' is owned by another resource.`, retryable: false });
    const body = { type: required.type, name: required.hostname, content: required.value, ttl: required.ttl === 'auto' ? 1 : required.ttl, proxied: false };
    ...
    const next: DnsRecordObservation = { provider: 'cloudflare', id: data.id ?? current?.id ?? '', zoneId, name: data.name ?? required.hostname, type: data.type ?? required.type, content: data.content ?? required.value, ttl: data.ttl ?? body.ttl, proxied: data.proxied === true, ownershipFingerprint };
