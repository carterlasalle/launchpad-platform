import type { DesiredApplication, PlannedOperation, PolicyResult } from './types.js';

export function evaluatePolicies(desired: DesiredApplication, operations: PlannedOperation[]): PolicyResult[] {
  const results: PolicyResult[] = [];
  const destructive = operations.filter((operation) => operation.destructive || operation.action === 'DESTROY');
